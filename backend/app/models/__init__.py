...
from sqlalchemy import (
    ...
    DateTime, ForeignKey, UniqueConstraint, BigInteger
)
...
class Investigation(Base):
    ...
    triggering_event_id = Column(UUID(as_uuid=True), nullable=True)
    assigned_analyst_id = Column(UUID(as_uuid=True), ForeignKey("users.user_id"), nullable=True)
    created_at = Column(DateTime(timezone=True), default=datetime.utcnow, nullable=False)
...
class Evidence(Base):
    ...
    entity_refs = Column(JSONB, nullable=True)  # {"hosts":[], "users":[], "ips":[]}
    produced_by_worker = Column(String(30), nullable=True)
    corroborated_by = Column(ARRAY(UUID(as_uuid=True)), default=list)
    created_at = Column(DateTime(timezone=True), default=datetime.utcnow, nullable=False)
...
class TimelineEvent(Base):
    ...
    investigation_id = Column(UUID(as_uuid=True), ForeignKey("investigations.investigation_id", ondelete="CASCADE"), nullable=False)
    evidence_id = Column(UUID(as_uuid=True), ForeignKey("evidence.evidence_id"), nullable=True)
...
class Hypothesis(Base):
    ...
    mitre_technique_ids = Column(ARRAY(String), default=list)
    created_at = Column(DateTime(timezone=True), default=datetime.utcnow, nullable=False)
    updated_at = Column(DateTime(timezone=True), default=datetime.utcnow, onupdate=datetime.utcnow, nullable=False)
...
class HypothesisEvidence(Base):
    ...
    hypothesis_id = Column(UUID(as_uuid=True), ForeignKey("hypotheses.hypothesis_id", ondelete="CASCADE"), primary_key=True)
    evidence_id = Column(UUID(as_uuid=True), ForeignKey("evidence.evidence_id", ondelete="CASCADE"), primary_key=True)
...
class WorkerExecution(Base):
    ...
    decision_id = Column(UUID(as_uuid=True), ForeignKey("planner_decisions.decision_id", ondelete="CASCADE"), nullable=False)
...
class AuditLog(Base):
    ...
    investigation_id = Column(UUID(as_uuid=True), ForeignKey("investigations.investigation_id"), nullable=True)
